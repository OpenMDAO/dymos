...
class StaticInterpolant(object):
    """
    StaticInterpolant has a similar interface to LagrangeBarycentricInterpolant, but is
    used to provide 'interpolated' values to constant functions.  That is, it looks like
    an interpolant but really is just returning fixed values and zero rates for a variable.
    ...
    ----------
    num_nodes : int
        The number of nodes in the interpolated LGL polynomial.
    tau_i : np.array
        The location of the nodes of the polynomial in tau space [-1 1]
    x_0 : float
        The x-value corresponding to the left-side of the interval (tau = -1)
    ...
    def __init__(self, shape):
        ...
        self._rate_val = np.zeros(shape)
        """ The value returned by eval_deriv """
    ...
    def setup(self, val):
        ...
        ----------
        val : float
            The fixed value to be returned by val at all nodes.
        xf : float
            The upper bound of the independent variable,
            corresponding to $\tau = -1$
        f_j : np.array
    ...
    def eval(self, *args):
        """ Interpolate the value (return the static value)

        Returns
        -------
        float
            The the value of the static control.

        """
        return self._val

    def eval_deriv(self, *args, **kwargs):
        """ Mimics the rate interpolation of the LagrangeBarycentricInterpolant, but always
        returns zero.
