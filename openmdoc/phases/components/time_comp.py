...
class TimeComp(ExplicitComponent):
    ...
    def initialize(self):
        ...
        # Optional
        self.metadata.declare('units', default=None, allow_none=True, types=string_types,
                              desc='Units of time (or the integration variable)')

        self.metadata.declare('internal', default=True, types=bool,
                              desc='If True, compute dt_dstau')

    def setup(self):
        time_units = self.metadata['units']
        node_ptau = self.metadata['grid_data'].node_ptau
        ...
        # Setup partials
        nn = self.metadata['grid_data'].num_nodes
        rs = np.arange(nn)
        cs = np.zeros(nn)
    ...
    def compute(self, inputs, outputs):
        node_ptau = self.metadata['grid_data'].node_ptau
        node_dptau_dstau = self.metadata['grid_data'].node_dptau_dstau
