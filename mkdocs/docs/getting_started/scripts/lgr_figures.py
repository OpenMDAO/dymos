...
prob.driver.add_recorder(om.SqliteRecorder('lgr_figures.db'))
...
phase = dm.Phase(ode_class=FallEOM, transcription=dm.Radau(num_segments=NUM_SEG, order=ORDER, compressed=False))
...
cr = om.CaseReader('lgr_figures.db')
...
for i, case_name in enumerate(cr.list_cases()):
    ...
    if i == 0:
        plt.savefig('lgr_animation_0.png')
    ...
    #
    # y_col = L.dot(y[state_disc_idxs])
    # vy_col = L.dot(vy[state_disc_idxs])
    #
    # yprime_col = D.dot(y[state_disc_idxs]) / dt_dstau[0]
    # vyprime_col = D.dot(vy[state_disc_idxs]) / dt_dstau[0]
    ...
    if i == 0:
        plt.savefig('lgr_animation_3.png')

    plt.savefig(f'lgr_solution_{i}.png')
