...
class TestBrachistochroneInvalidInterpolationState(unittest.TestCase):

    def test_interpolate_pre_setup(self):
        ...
        # Attempting to call phase.interpolate before setup should be an error

        with self.assertRaises(RuntimeError) as e:
            phase.interpolate(ys=[0, 10], nodes='state_input')

        self.assertEqual(str(e.exception),
                         'interpolate cannot be called until the associated problem has been setup')

    def test_interpolate_after_transcription_change(self):
        p = om.Problem(model=om.Group())
        ...
        phase.options['transcription'] = dm.Radau(num_segments=20, order=[3, 5]*10, compressed=True)
