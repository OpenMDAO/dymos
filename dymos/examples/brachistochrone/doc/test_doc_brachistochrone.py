...
class TestBrachistochroneForDocs(unittest.TestCase):
    ...
    def test_brachistochrone_for_docs_gauss_lobatto(self):
        ...
        p['traj.phase0.states:x'] = phase.interpolate('x', ys=[0, 10])
        p['traj.phase0.states:y'] = phase.interpolate('y', ys=[10, 5])
        p['traj.phase0.states:v'] = phase.interpolate('v', ys=[0, 9.9])
        p['traj.phase0.controls:theta'] = phase.interpolate('theta', ys=[5, 100.5])
