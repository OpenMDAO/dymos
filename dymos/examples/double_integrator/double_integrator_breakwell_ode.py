...
class DoubleIntegratorBreakwellODE(om.ExplicitComponent):
    ...
    def setup(self):
        ...
        self.add_input('v', val=np.zeros(nn))
        self.add_input('u', val=np.zeros(nn))

        self.add_output('x_dot', val=np.zeros(nn))
        self.add_output('v_dot', val=np.zeros(nn))
        self.add_output('J_dot', val=np.zeros(nn))

        ar = np.arange(nn)
        self.declare_partials(of='x_dot', wrt='v', rows=ar, cols=ar, val=1.0)
        self.declare_partials(of='v_dot', wrt='u', rows=ar, cols=ar, val=1.0)
