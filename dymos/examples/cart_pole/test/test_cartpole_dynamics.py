...
class TestCartPoleDynamics(unittest.TestCase):
    ...
    def test_cartpole_ode(self):
        ...
        # set input values
        p.model.set_input_defaults('m_cart', 1., units='kg')
        p.model.set_input_defaults('m_pole', 0.3, units='kg')
        p.model.set_input_defaults('l_pole', 0.5, units='m')
        p.model.set_input_defaults('theta', [0., 1.], units='rad')
        p.model.set_input_defaults('theta_dot', [0., 0.1], units='rad/s')
        ...
        ### print('x_dotdot:', x_dotdot)
        ### print('theta_dotdot:', theta_dotdot)
        assert_near_equal(x_dotdot, [10., -7.14331021], tolerance=1e-6)
        assert_near_equal(theta_dotdot, [-20., -8.79056677], tolerance=1e-6)
