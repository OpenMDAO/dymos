...
def animate_cartpole(x, theta, force, interval=20, force_scaler=0.1, save_gif=False, gif_fps=20):
    ...
    y_pole = -lpole * np.cos(theta) + cart_height / 2.
    ...
    def animate(i):
        ...
        ax.set_ylim(ylim)
        # plot cart
        cart = ax.add_patch(patches.Rectangle((-cart_width / 2. + x[i], 0.), cart_width, cart_height, edgecolor='C0', linewidth=1, fill=False))
        # plot pole
        pole = ax.plot([x[i], x_pole[i]], [cart_height / 2, y_pole[i]], 'o-', lw=2, color='C0')
        # plot pole tip path
        path = ax.plot(x_pole[:i], y_pole[:i], '--', lw=1, color='black')
        # plot force vector
        f = ax.arrow(arrow_origin[i], arrow_y, arrow_direc[i], 0., color='C1', head_width=0.02, head_length=0.03, length_includes_head=True)
