...
class CombinedODE(om.Group):
    ...
    def setup(self):
        ...
        self.add_subsystem(name='normal',
                           subsys=NormalForceODE(num_nodes=nn),promotes_inputs=['ax','ay','V'],promotes_outputs=['N_fr','N_fl','N_rr','N_rl'])

        self.add_subsystem(name='tire',
                           subsys=TireODE(num_nodes=nn),promotes_inputs=['lambda','omega','V','N_fr','N_fl','N_rr','N_rl','thrust','delta'],promotes_outputs=['S_fr','S_fl','S_rr','S_rl','F_fr','F_fl','F_rr','F_rl'])
        ...
        self.add_subsystem(name='car',
                           subsys=CarODE(num_nodes=nn),promotes_inputs=['n','alpha','V','lambda','omega','S_fr','S_fl','S_rr','S_rl','F_fr','F_fl','F_rr','F_rl','delta'],promotes_outputs=['sdot','ndot','alphadot','omegadot','Vdot','lambdadot','power'])
