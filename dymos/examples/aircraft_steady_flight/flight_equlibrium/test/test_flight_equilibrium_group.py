...
class TestFlightEquilibriumGroup(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        ivc.add_output('W_total', val=200000 * 9.80665 * np.ones(cls.n), units='N', desc='aircraft total weight')
    ...
    def test_results(self):
        ...
        assert_rel_error(self,  CL_eq, CL)
        assert_rel_error(self,  CM, np.zeros_like(CM))

    # @unittest.skip('skip for now')
    def test_partials(self):
        np.set_printoptions(linewidth=1024)
        cpd = self.p.check_partials(suppress_output=False, method='fd', step=1.0E-8)
        assert_check_partials(cpd, atol=2.0E-5, rtol=2.0)
