...
class TestAircraftCruise(unittest.TestCase):
    ...
    def test_cruise_results(self):
        ...
        txs = {'gauss-lobatto': dm.GaussLobatto(num_segments=1,
                                                order=13,
                                                compressed=False),
               'gauss-lobatto-compressed': dm.GaussLobatto(num_segments=2,
                                                           order=7,
                                                           compressed=True),
               'radau': dm.Radau(num_segments=2,
                                 order=13,
                                 compressed=False),
               'radau-compressed': dm.Radau(num_segments=2,
                                            order=7,
                                            compressed=True),
               'birkhoff': dm.Birkhoff(num_nodes=13,
                                       compressed=False)}

        for tx_name, tx in txs.items():
            with self.subTest(msg=f'{tx_name=}'):
                ...
                # transcription = dm.Birkhoff(num_nodes=50)
                # transcription = dm.Radau(order=3, num_segments=10, compressed=False)
                phase = dm.Phase(ode_class=AircraftODE, transcription=tx)
                traj = dm.Trajectory()
                ...
                phase.set_time_val(initial=0, duration=3600.0)

                phase.set_state_val('range', [0.0, 500.0])
                phase.set_state_val('mass_fuel', [10_000.0, 0.0])
                ...
                r = p.get_val('traj.phase0.timeseries.range')
                mass_fuel = p.get_val('traj.phase0.timeseries.mass_fuel')

                assert_near_equal(r, tas*time, tolerance=1.0E-4)
                ...
                r = case.get_val('traj.phase0.timeseries.range')
