...
class AverageAltitudeComp(ExplicitComponent):
    ...
    def setup(self):
        ...
        # Inputs
        self.add_input('alt', shape=(nn,), desc='altitude', units='m')

        # Outputs
        self.add_output('avg_alt', shape=(1,), desc='average altitude', units='m')
    ...
    def compute(self, inputs, outputs):
        nn = self.options['num_nodes']
        outputs['avg_alt'] = np.sum(inputs['alt']/nn)
...
def min_time_climb(optimizer='SLSQP', num_seg=3, transcription='gauss-lobatto',
    ...
    p.model.add_subsystem('avg_alt_comp', subsys=AverageAltitudeComp(num_nodes=46))
    p.model.connect('phase0.states:h', 'avg_alt_comp.alt')
    ...
    print('average altitude', p['avg_alt_comp.avg_alt'])
...
if __name__ == '__main__':
    SHOW_PLOTS = True
    p = min_time_climb(
        optimizer='SNOPT', num_seg=15, transcription='radau-ps', transcription_order=3,
        force_alloc_complex=False)
