...
class TestSimulationPhaseRadau(unittest.TestCase):
    ...
    @classmethod
    def tearDownClass(cls):
        for filename in ['phase0_sim_radau.sql']:
            if os.path.exists(filename):
                os.remove(filename)
    ...
    def setUpClass(cls):
        ...
        # Setup simulation manually
        cls.sim_prob = phase.simulate(record_file='phase0_sim_radau.sql')

    def test_recorded_simulate_data(self):
        sim_prob = self.sim_prob

        cr = CaseReader('phase0_sim_radau.sql')
        case = cr.get_case(cr.list_cases()[0])
...
class TestSimulationPhaseRK4(unittest.TestCase):

    @classmethod
    def tearDownClass(cls):
        for filename in ['phase0_sim_rk4.sql']:
            if os.path.exists(filename):
                os.remove(filename)
    ...
    def setUpClass(cls):
        ...
        # Setup simulation manually
        cls.sim_prob = phase.simulate(record_file='phase0_sim_rk4.sql')

    def test_recorded_simulate_data(self):
        sim_prob = self.sim_prob

        cr = CaseReader('phase0_sim_rk4.sql')
        case = cr.get_case(cr.list_cases()[0])
