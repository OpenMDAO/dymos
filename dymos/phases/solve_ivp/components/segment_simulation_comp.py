...
class SegmentSimulationComp(ExplicitComponent):
    ...
    def initialize(self):
        ...
        self.options.declare('atol', default=1.0E-9, types=(float,),
                             desc='Absolute tolerance passed to scipy.integrate.solve_ivp.')

        self.options.declare('rtol', default=1.0E-9, types=(float,),
    ...
    def setup(self):
        ...
        self.add_input(name='t0_seg', val=0.0, units=self.options['time_options']['units'],
                       desc='Initial time of the segment.')

        self.add_input(name='tf_seg', val=1.0, units=self.options['time_options']['units'],
                       desc='Final time of the segment')
        ...
        for name, options in iteritems(self.options['state_options']):
            ...
            self.add_output(name='final_states:{0}'.format(name),
                            val=np.ones((1,) + options['shape']),
                            units=options['units'],
                            desc='Final value of state {0} in the segment.'.format(name))
    ...
    def compute(self, inputs, outputs, discrete_inputs=None, discrete_outputs=None):
        ...
        sol = solve_ivp(fun=self.ode_integration_interface,
                        t_span=(inputs['t0_seg'], inputs['tf_seg']),
                        y0=self.initial_state_vec,
                        method=self.options['method'],
                        atol=self.options['atol'],
                        rtol=self.options['rtol'],
                        t_eval=np.linspace(inputs['t0_seg'][0], inputs['tf_seg'][0], 100))
        ...
        for name, options in iteritems(self.options['state_options']):
            size = np.prod(options['shape'])
            # TODO: Eventually we need to return states at evaluation nodes
            outputs['final_states:{0}'.format(name)] = sol.y[pos:pos+size, :].T[-1, ...]
            pos += size
