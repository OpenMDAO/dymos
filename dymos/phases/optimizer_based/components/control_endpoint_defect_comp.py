...
class ControlEndpointDefectComp(ExplicitComponent):
    ...
    def setup(self):
        ...
        for control_name, options in iteritems(control_options):
            ...
            row_idxs = np.repeat(np.arange(size, dtype=int), gd.subset_num_nodes_per_segment['all'][-1])

            initial_col = sum([gd.subset_num_nodes_per_segment['all'][i] * size for i in range(gd.num_segments - 1)])
            final_col = sum([gd.subset_num_nodes_per_segment['all'][i] * size for i in range(gd.num_segments)])
