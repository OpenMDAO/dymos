...
class ControlRateComp(ExplicitComponent):
    ...
    def initialize(self):
        ...
        self._output_norm_rate_names = {}
        self._output_norm_rate2_names = {}
    ...
    def _setup_controls(self):
        ...
        for name, options in iteritems(control_options):
            ...
            self._output_norm_rate_names[name] = 'control_rates:{0}_norm_rate'.format(name)
            self._output_norm_rate2_names[name] = 'control_rates:{0}_norm_rate2'.format(name)
            shape = options['shape']
            units = options['units']
            ...
            if options['dynamic']:
                ...
                self.add_output(self._output_norm_rate_names[name], shape=io_shape, units=units)

                self.add_output(self._output_norm_rate2_names[name], shape=io_shape, units=units)
                ...
                #
                # partials[norm_rate_name, control_name] = \
                #     (self.jacs[name])[r_nz, c_nz]

                self.declare_partials(of=self._output_norm_rate_names[name],
                                      wrt=self._input_names[name],
                                      rows=self.jac_rows[name], cols=self.jac_cols[name],
                                      val=self.jacs[name][self.jac_rows[name], self.jac_cols[name]])
                ...
                self.declare_partials(of=self._output_norm_rate2_names[name],
                                      wrt=self._input_names[name],
                                      rows=self.jac2_rows[name], cols=self.jac2_cols[name],
                                      val=self.jacs2[name][self.jac2_rows[name],
                                                           self.jac2_cols[name]])
                ...
                self.add_output(self._output_norm_rate_names[name],
                                val=np.zeros(io_shape),
                                units=rate_units)

                self.add_output(self._output_norm_rate2_names[name],
                                val=np.zeros(io_shape),
                                units=rate2_units)
    ...
    def compute(self, inputs, outputs):
        ...
        for name, options in iteritems(control_options):
            ...
            if options['dynamic']:
                ...
                outputs[self._output_norm_rate_names[name]] = a.T
                outputs[self._output_norm_rate2_names[name]] = b.T
